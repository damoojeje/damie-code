...
export function SettingsDialog({
  ...
  const handleProviderConfigSave = (
    providerKey: string,
    config: { apiKey?: string; baseUrl?: string; model?: string; timeout?: number },
  ) => {
    // Update pending settings with provider configuration
    const newPendingSettings = { ...pendingSettings };
    if (!newPendingSettings.security) {
      newPendingSettings.security = {};
    }
    if (!newPendingSettings.security.providers) {
      (newPendingSettings.security as unknown as { providers: Record<string, unknown> }).providers = {};
    }
    (newPendingSettings.security as unknown as { providers: Record<string, unknown> }).providers[providerKey] = config;
    setPendingSettings(newPendingSettings);

    // Mark as modified and requiring restart
    const settingKey = `security.providers.${providerKey}`;
    setModifiedSettings((prev) => new Set(prev).add(settingKey));
    setRestartRequiredSettings((prev) => new Set(prev).add(settingKey));
  ...
  useKeypress(
    (key) => {
      // Ignore keypresses when provider config dialog is open
    ...
    <>
      {showProviderConfig && (
        <ProviderConfigDialog
          ...
          onExit={() => setShowProviderConfig(false)}
        />
      )}
    ...
    >
      <Box flexDirection="column" flexGrow={1}>
        ...
        <Text color={theme.text.secondary}>
          (Use Enter to select
          {showScopeSelection ? ', Tab to change focus' : ''}, Press 'p' for Provider Config)
        </Text>
        {showRestartPrompt && (
          <Text color={theme.status.warning}>
            To see changes, Damie Code must be restarted. Press r to exit and
            apply changes now.
          </Text>
        )}
      </Box>
