...
import * as ServerConfig from '@damie-code/damie-code-core';
...
vi.mock('@damie-code/damie-code-core', async () => {
  const actualServer = await vi.importActual<typeof ServerConfig>(
    '@damie-code/damie-code-core',
  );
  return {
    ...actualServer,
    IdeClient: {
...
describe('Hierarchical Memory Loading (config.ts) - Placeholder Suite', () => {
    ...
    const extensions: Extension[] = [
      {
        ...
        contextFiles: ['/path/to/ext1/DAMIE.md'],
      },
      {
        path: '/path/to/ext2',
        config: {
  ...
  it.skip('should correctly use mocked homedir for global path', async () => {
    const MOCK_GEMINI_DIR_LOCAL = path.join('/mock/home/user', '.damie');
    const MOCK_GLOBAL_PATH_LOCAL = path.join(MOCK_GEMINI_DIR_LOCAL, 'DAMIE.md');
    mockFs({
