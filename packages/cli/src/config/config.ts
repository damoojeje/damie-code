...
export async function parseArguments(settings: Settings): Promise<CliArgs> {
  ...
  const yargsInstance = yargs(rawArgv)
    ...
    .scriptName('damie')
    .usage(
      'Usage: damie [options] [command]\n\nDamie Code - Launch an interactive CLI, use -p/--prompt for non-interactive mode',
    )
    .option('telemetry', {
      type: 'boolean',
      description:
    ...
    .option('proxy', {
      type: 'string',
      description:
        'Proxy for API client, like schema://user:password@host:port',
    })
    .deprecateOption(
      'proxy',
      'Use the "proxy" setting in settings.json instead. This flag will be removed in a future version.',
    )
    .command('$0 [query..]', 'Launch Damie Code', (yargsInstance: Argv) =>
      yargsInstance
        .positional('query', {
          description:
