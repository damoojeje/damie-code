...
describe('computeWindowTitle', () => {
  ...
  it('should use default Damie title when CLI_TITLE is not set', () => {
    const result = computeWindowTitle('my-project');
    expect(result).toBe('Damie - my-project');
  });
  ...
  it('should handle folder names with special characters', () => {
    ...
    expect(result).toBe('Damie - project-name_v1.0');
  });
});
