...
import {
  IDE_DEFINITIONS,
  detectIdeFromEnv,
} from '@damie-code/damie-code-core/src/ide/detect-ide.js';

vi.mock('@damie-code/damie-code-core/src/ide/detect-ide.js', async () => {
  const actual = await vi.importActual(
    '@damie-code/damie-code-core/src/ide/detect-ide.js',
  );
  return {
    ...actual,
...
describe('activate', () => {
  ...
  it('should show the info message on first activation', async () => {
    ...
    expect(showInformationMessageMock).toHaveBeenCalledWith(
      'Damie Code Companion extension successfully installed.',
    );
  });
  ...
  it('should launch the Damie Code when the user clicks the button', async () => {
    const showInformationMessageMock = vi
      .mocked(vscode.window.showInformationMessage)
      .mockResolvedValue('Run Damie Code' as never);
    vi.mocked(context.globalState.get).mockReturnValue(undefined);
    vi.mocked(vscode.extensions.getExtension).mockReturnValue({
      packageJSON: { version: '1.1.0' },
    } as vscode.Extension<unknown>);
  ...
  describe('update notification', () => {
    ...
    it('should show an update notification if a newer version is available', async () => {
      ...
      expect(showInformationMessageMock).toHaveBeenCalledWith(
        'A new version (1.2.0) of the Damie Code Companion extension is available.',
