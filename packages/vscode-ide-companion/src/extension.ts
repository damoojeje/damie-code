  ...
  try {
    ...
    if (latestVersion && semver.gt(latestVersion, currentVersion)) {
      const selection = await vscode.window.showInformationMessage(
        `A new version (${latestVersion}) of the Damie Code Companion extension is available.`,
...
export async function activate(context: vscode.ExtensionContext) {
  logger = vscode.window.createOutputChannel('Damie Code Companion');
    ...
    vscode.commands.registerCommand('qwen-code.runQwenCode', async () => {
      ...
      if (!workspaceFolders || workspaceFolders.length === 0) {
        vscode.window.showInformationMessage(
          'No folder open. Please open a folder to run Damie Code.',
        );
        return;
      }
      ...
      if (workspaceFolders.length === 1) {
        ...
        selectedFolder = await vscode.window.showWorkspaceFolderPick({
          placeHolder: 'Select a folder to run Damie Code in',
        });
      }
      ...
      if (selectedFolder) {
        ...
        const terminal = vscode.window.createTerminal({
          name: `Damie Code (${selectedFolder.name})`,
