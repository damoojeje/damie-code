...
export class IdeClient {
  ...
  async connect(): Promise<void> {
    if (!this.currentIde) {
      this.setState(
        ...
        `IDE integration is not supported in your current environment. To use this feature, run Damie Code in one of these supported IDEs: VS Code or VS Code forks`,
        false,
      );
      return;
    }
  ...
  static validateWorkspacePath(
    ...
    if (!isWithinWorkspace) {
      return {
        isValid: false,
        error: `Directory mismatch. Damie Code is running in a different location than the open workspace in the IDE. Please run the CLI from one of the following directories: ${ideWorkspacePaths.join(
          ', ',
        )}`,
      };
    }
