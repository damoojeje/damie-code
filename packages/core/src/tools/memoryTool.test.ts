...
const MEMORY_SECTION_HEADER = '## Damie Added Memories';
  ...
  describe('shouldConfirmExecute', () => {
    ...
    it('should return confirmation details when memory file is not allowlisted for global scope', async () => {
      ...
      if (result && result.type === 'edit') {
        const expectedPath = path.join('~', '.damie', 'DAMIE.md');
        expect(result.title).toBe(
          `Confirm Memory Save: ${expectedPath} (global)`,
        );
        expect(result.fileName).toContain(path.join('mock', 'home', '.damie'));
        expect(result.fileName).toContain('DAMIE.md');
        expect(result.fileDiff).toContain('Index: DAMIE.md');
        expect(result.fileDiff).toContain('+## Damie Added Memories');
        expect(result.fileDiff).toContain('+- Test fact');
        expect(result.originalContent).toBe('');
        expect(result.newContent).toContain('## Damie Added Memories');
    ...
    it('should return confirmation details when memory file is not allowlisted for project scope', async () => {
      ...
      if (result && result.type === 'edit') {
        const expectedPath = path.join(process.cwd(), 'DAMIE.md');
        expect(result.title).toBe(
    ...
    it('should prompt for scope selection when scope is not specified', async () => {
      ...
      if (result && result.type === 'edit') {
        ...
        expect(result.fileName).toBe('DAMIE.md');
        expect(result.fileDiff).toContain('Test fact');
        expect(result.fileDiff).toContain('--- DAMIE.md');
        expect(result.fileDiff).toContain('+++ DAMIE.md');
        expect(result.fileDiff).toContain('+- Test fact');
    ...
    it('should show correct file paths in scope selection prompt', async () => {
      ...
      if (result && result.type === 'edit') {
        const globalPath = path.join('~', '.damie', 'DAMIE.md');
        const projectPath = path.join(process.cwd(), 'DAMIE.md');
