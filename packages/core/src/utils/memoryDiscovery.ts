...
async function getGeminiMdFilePathsInternalForEachDir(
  ...
  for (const geminiMdFilename of geminiMdFilenames) {
    ...
    if (isHomeDirectory) {
      // For home directory, only check for DAMIE.md directly in the home directory
...
/**
 * Loads hierarchical DAMIE.md files and concatenates their content.
...
export async function loadServerHierarchicalMemory(
  ...
  if (filePaths.length === 0) {
    if (debugMode) logger.debug('No DAMIE.md files found in hierarchy.');
